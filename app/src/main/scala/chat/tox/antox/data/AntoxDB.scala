...
class AntoxDB(ctx: Context, activeDatabase: String, selfKey: SelfKey) {
  ...
  def unreadMsgList(): ArrayBuffer[Message] = {
    ...
    val whereKey =
      s"WHERE $TABLE_MESSAGES.$COLUMN_NAME_HAS_BEEN_READ == $FALSE"
  ...
  // modified by Ann on 12/31/2015, setMessageEvaled
  def setMessageEvaled(messageId: Int) {
    val where = s"$COLUMN_NAME_MESSAGE_ID ='$messageId'"
  ...
  // modified by Ann on 12/31/2015, get all keys of Friend
  def getAllFriendKey(): ArrayBuffer[FriendKey] = {
    val query = s"SELECT $COLUMN_NAME_KEY FROM $TABLE_CONTACTS"
    // val allFriendKey = new ArrayBuffer[FriendKey]()
    // val friendKey: FriendKey = _
    val allFriendKey = mDb.query(query).use { cursor =>
      if (cursor.moveToFirst()) {
        getAllFriendKeyFromCursor(cursor)
      } else {
        throw new IllegalArgumentException(s"Friends not found.")
      }
    }
    allFriendKey
  }

  private def getAllFriendKeyFromCursor(cursor: Cursor): ArrayBuffer[FriendKey] = {
    val allFriendKey = new ArrayBuffer[FriendKey]()
    do {
      allFriendKey += new FriendKey(cursor.getString(COLUMN_NAME_KEY))
    } while (cursor.moveToNext())
    allFriendKey
  }
  // modified by Ann on 12/31/2015
