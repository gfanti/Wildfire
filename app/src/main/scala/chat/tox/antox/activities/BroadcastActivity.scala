...
class BroadcastActivity extends GenericBroadcastActivity {
  ...
  override def onCreate(savedInstanceState: Bundle): Unit = {
        ...
        fileDialog.addFileListener(new FileDialog.FileSelectedListener() {
          def fileSelected(file: File) {
            for (friendKey <- ToxSingleton.tox.getFriendList) {
              State.transfers.sendFileSendRequest(file.getPath, friendKey, FileKind.DATA, ToxFileId.empty, thisActivity)
            }
          }
        })
    ...
    cameraButton.setOnClickListener(new View.OnClickListener() {
      override def onClick(v: View) {
        /*var friendInfo: FriendInfo = _
        var flag = false
        for (friendKey <- ToxSingleton.tox.getFriendList) {
          friendInfo = State.db.getFriendInfo(friendKey)
          if (friendInfo.online) {
            flag = true
          }
        }
  ...
  override def onActivityResult(requestCode: Int, resultCode: Int, data: Intent) {
    ...
    if (resultCode == Activity.RESULT_OK) {
      if (requestCode == Constants.IMAGE_RESULT) {
        ...
        if (cursor != null) {
          if (cursor.moveToFirst()) {
            ...
            try {
              for (friendKey <- ToxSingleton.tox.getFriendList) {
                State.transfers.sendFileSendRequest(filePath, friendKey, FileKind.DATA, ToxFileId.empty, this)
              }
            } catch {
      ...
      if (requestCode == Constants.PHOTO_RESULT) {
        try {
          for (friendKey <- ToxSingleton.tox.getFriendList) {
            photoPath.foreach(path => State.transfers.sendFileSendRequest(path, friendKey, FileKind.DATA, ToxFileId.empty, this))
            photoPath = None
          }
        } catch {
          case e: Exception => e.printStackTrace()
  ...
  // Message Diffusion Scheme Implementation
  // Current scheme: broadcast
  override def sendMessage(message: String, messageType: ToxMessageType, context: Context): Unit = {
    for (friendKey <- ToxSingleton.tox.getFriendList) {
      MessageHelper.sendMessage(this, friendKey, message, messageType, None)
    }
  }
  // Message Diffusion Scheme Implementation
}
